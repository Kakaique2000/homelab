#!/bin/bash
# Configures WireGuard on a K8s node and registers it as a peer on the VPS.
# After running this, add the node to NODES in edge.conf and run ./deploy.sh.
#
# Usage:
...
if [ -z "$NODE_NAME" ] || [ -z "$NODE_IP" ] || [ -z "$WG_IP" ]; then
  echo "Usage: $0 <node-name> <node-local-ip> <wg-ip>"
  echo "  Example: $0 k8s-master 192.168.15.13 10.8.0.2"
  exit 1
fi
...
# ============================================================
# Step 1: Install WireGuard on node
# ============================================================
echo "[1/4] Installing WireGuard on node..."
nssh "sudo apt-get install -y wireguard-tools -qq"
echo "  ✓ wireguard-tools installed"
echo ""

# ============================================================
# Step 2: Generate keypair on node
# ============================================================
echo "[2/4] Generating WireGuard keypair on node..."
NODE_PUBKEY=$(nssh "
...
# ============================================================
# Step 3: Configure WireGuard tunnel on node
# ============================================================
echo "[3/4] Configuring WireGuard on node..."
...
# ============================================================
echo "[4/4] Registering node as WireGuard peer on VPS..."
vssh "
  sudo mkdir -p /etc/wireguard/peers
  sudo tee /etc/wireguard/peers/${NODE_NAME}.conf > /dev/null << PEEREOF
[Peer]
# ${NODE_NAME}
...
"
echo "  ✓ Peer added to VPS (live, no restart)"
echo ""
...
if vssh "ping -c 3 -W 3 ${WG_IP}" > /dev/null 2>&1; then
  ...
  echo "  WARNING: VPS cannot ping $WG_IP yet — may need a moment"
fi
echo ""

echo "=== Done ==="
echo ""
echo "Now add the node to edge.conf:"
echo "  NODES=("
echo "    \"${NODE_NAME}:${WG_IP}\""
echo "  )"
echo ""
